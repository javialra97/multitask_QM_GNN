...
for i in range(args.k_fold):
    ...
    if args.data_path is not None:
        ...
        ].sample(frac=1 / (args.k_fold - 1), random_state=args.random_state)
        train = df[
            ~(
                df[f"{args.rxn_id_column}"].isin(test[f"{args.rxn_id_column}"])
    ...
    for x, y in tqdm(test_gen, total=int(len(test_smiles) / args.selec_batch_size)):
        ...
        for y_output, y_true in zip(out["activation_energy"], y["activation_energy"]):
            ...
            mae_activation_energy += abs(activation_energy_predicted - y_true) / int(
                len(test_smiles)
            )
            mse_activation_energy += (activation_energy_predicted - y_true) ** 2 / int(
                len(test_smiles)
            )
        for y_output, y_true in zip(out["reaction_energy"], y["reaction_energy"]):
            ...
            mae_reaction_energy += abs(reaction_energy_predicted - y_true) / int(
                len(test_smiles)
            )
            mse_reaction_energy += (reaction_energy_predicted - y_true) ** 2 / int(
                len(test_smiles)
            )
    ...
    test_predicted = pd.DataFrame(
        {
            f"{args.rxn_id_column}": test_rxn_id,
    ...
    print(
        f"success rate for iter {i} - activation energy: {rmse_activation_energy}, {mae_activation_energy} - reaction energy: {rmse_reaction_energy}, {mae_reaction_energy}"
    )
    logger.info(
        f"success rate for iter {i} - activation energy: {rmse_activation_energy}, {mae_activation_energy} - reaction energy: {rmse_reaction_energy}, {mae_reaction_energy}"
    )
